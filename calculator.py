    ...
    def lines_to_cases(self, input):
        ...
        while left_number_of_cases > 0:
            ...
            for line in lines[line_index+1 : next_line_index]:
                points.append(self.line_to_point(line))
            cases.append(points)
            line_index = next_line_index
            left_number_of_cases -= 1
        return cases
    
    def line_to_point(self, text):
        cordinate = text.split(" ")
        return Point(cordinate[0], cordinate[1])

def main():
    print("Start executing...")
    cases = Converter().lines_to_cases(input())
    for case in cases:
        Calculator().closest_distance(case)
